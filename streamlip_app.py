...
# Heuristic for space type from room name
...
def generate_comcheck(fixtures_csv: str, spaces_csv: str) -> str:
    # --- Parse spaces.csv into ordered list ---
    spaces = {}
    rooms = []
    ...
    for row in reader_s:
        ...
        spaces[room] = sqft
        rooms.append(room)
    # map room -> index for list position
    ...
    for room in rooms:
        ...
        for fix, qty in fixtures[room].items():
            ...
            # Decorative allowances per (room,fix)
            key = (room, fix)
            if key in allowance_map:
                alw = allowance_map[key]
                lines.extend([
